...
export async function POST(req: NextRequest) {
  try {
    ...
    const { id, status, reviewer_comment, featured } = body
    if (!id) return NextResponse.json({ error: 'id 누락' }, { status: 400 })

    const update: Record<string, unknown> = {}
    if (status !== undefined) update.status = status
    if (reviewer_comment !== undefined) update.reviewer_comment = reviewer_comment
    ...
    const { error } = await admin.from('contents').update(update).eq('id', id)
